...
export function AppList() {
  ...
  const { data: recordings = [] } = useSWRSubscription<RecordingStatus[]>(
    'recordings',
    (_key: string, { next }: { next: (err: Error | null, data?: RecordingStatus[]) => void }) => {
    ...
    const mapping = apps.reduce((acc: Record<number, AppGroup>, app: App) => {
      if (!acc[app.processGroupId]) {
        acc[app.processGroupId] = {
          ...
          apps: [],
          rootApp: apps.find((a: App) => a.processId === app.processGroupId) || app,
        }
      }
    ...
    <div className="h-full flex flex-col divide-y divide-gray-100">
      <div className="p-4 relative">
        <div className="flex items-center justify-between sticky top-0 bg-white z-10 mb-2">
          <h2 className="text-sm font-semibold text-gray-900">Active Applications</h2>
        ...
        <div className="space-y-2">
          {runningApps.map((app) => (
            <AppItem key={app.processGroupId} app={app} recordings={recordings} />
          ))}
      ...
      <div className="p-4 flex-1 relative">
        <div className="flex items-center justify-between sticky top-0 bg-white z-10 mb-2">
          <h2 className="text-sm font-semibold text-gray-900">Other Applications</h2>
