...
import { type ApplicationInfo, type AudioCaptureSession, ShareableContent } from '@recappi/sdk'
...
async function saveRecording(recording: Recording, sampleRate: number, channels: number): Promise<string | null> {
  try {
    recording.isWriting = true
    const app = recording.isGlobal ? null : recording.appGroup || recording.app

    const totalSamples = recording.buffers.reduce((acc, buf) => acc + buf.length, 0)
    ...
    console.log(`- Sample ratio: ${(totalSamples / expectedSamples).toFixed(2)}`)
    ...
    const transcriptionWavFilename = path.join(recordingDir, 'transcription.wav')
    ...
    console.log(`📝 Writing transcription wav file to ${transcriptionWavFilename}`)
...
async function startRecording(app: ApplicationInfo) {
  ...
  if (recordingMap.has(appProcessId)) {
    console.log(`⚠️ Recording already in progress for ${appName} (PID: ${appProcessId})`)
    return
  }

  try {
    console.log(`🎙️ Starting recording for ${appName} (Bundle: ${appBundleId}, PID: ${appProcessId})`)
    ...
    const session = ShareableContent.tapAudio(appProcessId, (err: any, samples: any) => {
      if (err) {
...
async function stopRecording(processId: number) {
  ...
  const appName = recording.isGlobal ? 'Global Recording' : getAppName(app) || 'Unknown App'
  const appPid = getAppProcessId(app)

  console.log(`⏹️ Stopping recording for ${appName} (PID: ${appPid})`)
  console.log(`⏱️ Recording duration: ${((Date.now() - recording.startTime) / 1000).toFixed(2)}s`)
...
> {
  try {
    ...
    return recordings
      ...
      .sort((a, b) => (b.metadata?.recordingStartTime ?? 0) - (a.metadata?.recordingStartTime ?? 0))
  } catch (error) {
...
async function getAllApps(): Promise<AppInfo[]> {
  ...
  const filteredApps = apps.filter((v): v is AppInfo => {
    return v !== null && (!v.bundleIdentifier.startsWith('com.apple') || v.bundleIdentifier === 'com.apple.Music')
  })
  ...
  const subscribers = apps.map(({ app }) => {
    try {
      ...
      const onAppStateChanged = () => {
        const currentIsRunning = ShareableContent.isUsingMicrophone(appProcessId)
      ...
      return ShareableContent.onAppStateChanged(app, debounce(onAppStateChanged, 500))
    } catch (error) {
      console.error(`Failed to listen to app state changes for ${app ? getAppName(app) : 'unknown app'}:`, error)
      return { unsubscribe: () => {} }
    }
  })
...
app.delete('/recordings/:foldername', rateLimiter, (async (req: Request, res: Response): Promise<void> => {
  const foldername = validateAndSanitizeFolderName(req.params.foldername as string)
  if (!foldername) {
...
app.post('/recordings/:foldername/transcribe', rateLimiter, (async (req: Request, res: Response): Promise<void> => {
  const foldername = validateAndSanitizeFolderName(req.params.foldername as string)
  if (!foldername) {
    console.error('❌ Invalid folder name format:', req.params.foldername)
    res.status(400).json({ error: 'Invalid folder name format' })
  ...
  try {
    ...
    await fs.writeJson(`${recordingDir}/transcription.json`, metadata).catch((err) => {
...
async function startGlobalRecording() {
  ...
  try {
    ...
    const buffers: Float32Array[] = []
    const session = ShareableContent.tapGlobalAudio(null, (err: Error | null, samples: Float32Array) => {
      if (err) {
