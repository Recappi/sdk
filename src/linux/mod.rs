...
fn start_recording(
  ...
  let ffmpeg_args = build_capture_args()?;
  let mut child = Command::new("ffmpeg")
    ...
    .spawn()
    .map_err(|err| {
      linux_backend_error(CAPTURE_FEATURE, format!("Failed to launch ffmpeg: {err}"))
    })?;

  let stdout = child
    .stdout
    .take()
    .ok_or_else(|| linux_backend_error(CAPTURE_FEATURE, "ffmpeg stdout was not captured."))?;
  let stderr = child
    .stderr
    .take()
    .ok_or_else(|| linux_backend_error(CAPTURE_FEATURE, "ffmpeg stderr was not captured."))?;
  ...
  let stdout_thread = thread::spawn(move || {
    ...
    let _ = callback_for_thread.call(
      Err(linux_backend_error(CAPTURE_FEATURE, detail)),
      ThreadsafeFunctionCallMode::NonBlocking,
    );
  });
